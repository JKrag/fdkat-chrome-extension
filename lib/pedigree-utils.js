...
// Returns true if every occurrence in eCells falls within the row range of a *strictly shallower*
// cell in dCells (i.e. dCell spans more rows). Requiring strict containment prevents two
// same-depth duplicates from mutually suppressing each other.
function allCoveredBy(eCells, dCells) {
  return eCells.every(eCell =>
    dCells.some(dCell =>
      dCell.rowStart <= eCell.rowStart &&
      eCell.rowEnd <= dCell.rowEnd &&
      (dCell.rowEnd - dCell.rowStart) > (eCell.rowEnd - eCell.rowStart)
    )
  );
}
